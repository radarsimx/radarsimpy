...
def test_cw_tx():
    ...
    assert np.array_equal(cw.az_angles, [
                          np.arange(-90, 91, 180)])
    assert np.array_equal(cw.az_patterns, [
                          np.zeros(2)])
    assert np.array_equal(cw.el_angles, [
                          np.arange(-90, 91, 180)])
    assert np.array_equal(cw.el_patterns, [
                          np.zeros(2)])

    print('# CW transmitter modulation #')
    assert np.array_equal(cw.pulse_mod, [
                          np.ones(cw.pulses)])
...
def fmcw_tx():
    ...
    return Transmitter(f=[24.125e9-50e6, 24.125e9+50e6],
                       t=80e-6,
                       tx_power=10,
                       prp=100e-6,
                       pulses=256,
...
def tdm_fmcw_tx():
    ...
    tx_channel_1 = dict(location=(0, -4 * wavelength, 0),
                        delay=0)
...
def test_tdm_fmcw_tx():
    ...
    assert np.array_equal(tdm.locations, np.array([[0, -4*const.c/24.125e9, 0],
                                                   [0, 0, 0]]))
    assert np.array_equal(tdm.az_angles, [
                          np.arange(-90, 91, 180), np.arange(-90, 91, 180)])
    assert np.array_equal(tdm.az_patterns, [
                          np.zeros(2), np.zeros(2)])
    assert np.array_equal(tdm.el_angles, [
                          np.arange(-90, 91, 180), np.arange(-90, 91, 180)])
    assert np.array_equal(tdm.el_patterns, [
                          np.zeros(2), np.zeros(2)])

    print('# TDM FMCW transmitter modulation #')
    assert np.array_equal(tdm.pulse_mod, [
                          np.ones(tdm.pulses), np.ones(tdm.pulses)])
...
def test_pmcw_tx():
    ...
    assert np.array_equal(pmcw.locations, np.array([[0, 0, 0],
                                                    [0, 0, 0]]))
    assert np.array_equal(pmcw.az_angles, [
                          np.arange(-90, 91, 1), np.arange(-90, 91, 1)])
    assert np.array_equal(pmcw.az_patterns, [
                          np.zeros(181), np.zeros(181)])
    assert np.array_equal(pmcw.el_angles, [
                          np.arange(-90, 91, 1), np.arange(-90, 91, 1)])
    assert np.array_equal(pmcw.el_patterns, [
                          np.zeros(181), np.zeros(181)])
