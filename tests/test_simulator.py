...
def test_sim_cw():
    ...
    target = dict(location=('1.5+1e-3*math.sin(2*math.pi*1*t)', 0, 0),
                  rcs=0,
                  phase=0)
    targets = [target]
    ...
    npt.assert_almost_equal(
        f[np.argmax(spectrum[0:int(nfft/2)])], 1, decimal=2)
    npt.assert_almost_equal(timestamp[0, 0, :], np.arange(
        0, radar.samples_per_pulse)/radar.receiver.fs)

    target = dict(location=(1.5+1e-3*np.sin(2*np.pi*1*radar.timestamp), 0, 0),
                  rcs=0,
                  phase=0)
    targets = [target]
...
def test_sim_fmcw():
    ...
    rng_targets = np.sort(np.array([np.sqrt(target_1['location'][0]**2 +
                                            target_1['location'][1]**2),
                                    np.sqrt(target_2['location'][0]**2 +
                                            target_2['location'][1]**2),
                                    np.sqrt(target_3['location'][0]**2 +
                                            target_3['location'][1]**2)]))
    dop_targets = np.sort(np.array(
        [target_1['speed'][0]*np.cos(np.arctan(target_1['location'][1] /
                                               target_1['location'][0])),
         target_2['speed'][0] * np.cos(np.arctan(target_2['location'][1] /
                                                 target_2['location'][0])),
         target_3['speed'][0]*np.cos(np.arctan(target_3['location'][1] /
                                               target_3['location'][0]))]))
    ...
    assert np.array_equal((
        radar.channel_size*radar.frames, radar.transmitter.pulses,
        radar.samples_per_pulse), np.shape(timestamp))
    assert np.array_equal((
        radar.channel_size*radar.frames, radar.transmitter.pulses,
        radar.samples_per_pulse), np.shape(baseband))
    ...
    max_range = (const.c * radar.receiver.fs *
                 radar.transmitter.pulse_length /
                 radar.transmitter.bandwidth / 2)

    unambiguous_speed = const.c / \
        radar.transmitter.prp[0] / \
    ...
    rng_dets = np.sort(range_axis[rng_peaks])
    npt.assert_almost_equal(np.array([9.,  48., 195.]), rng_dets, decimal=0)
    ...
    dop_dets = np.sort(doppler_axis[dop_peaks])
    npt.assert_almost_equal(
        np.array([-45.66062176, -18.45854922,  -5.1003886]),
        dop_dets, decimal=0)
...
def test_sim_tdm_fmcw():
    ...
    assert np.array_equal((radar.channel_size, radar.transmitter.pulses,
                           radar.samples_per_pulse), np.shape(timestamp))
    assert np.array_equal((radar.channel_size, radar.transmitter.pulses,
                           radar.samples_per_pulse), np.shape(baseband))
    ...
    npt.assert_almost_equal(timestamp[0, :, 0], (np.arange(
        0, radar.transmitter.pulses)*radar.transmitter.prp[0]))
    npt.assert_almost_equal(timestamp[:, 0, 0], np.array([
        0, 0, 0, 0,
        0, 0, 0, 0,
    ...
    npt.assert_almost_equal(timestamp[:, 1, 0], np.array([
        200e-6, 200e-6, 200e-6, 200e-6,
        200e-6, 200e-6, 200e-6, 200e-6,
        300e-6, 300e-6, 300e-6, 300e-6,
        300e-6, 300e-6, 300e-6, 300e-6
...
def test_sim_pmcw():
    ...
    range_profile = np.zeros(
        (radar.channel_size, radar.transmitter.pulses, code_length),
        dtype=complex)
    ...
    unambiguous_speed = const.c / radar.transmitter.prp[0] / \
        radar.transmitter.fc_vect[0] / 2
