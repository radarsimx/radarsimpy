...
def test_fmcw_raytracing_tx_azimuth():
    ...
    tx = Transmitter(f=[10e9-50e6, 10e9+50e6],
                     t=[0, 80e-6],
                     tx_power=15,
                     prp=0.5,
                     pulses=1,
...
def test_fmcw_raytracing_radar_rotation():
    ...
    data = scene(radar, targets, density=1,
                 noise=False, level='pulse', debug=False)
...
def test_fmcw_raytracing_radar_speed():
    ...
    radar = Radar(transmitter=tx, receiver=rx, time=[
                  0, 1], location=(-20, 0, 0), speed=(20, 0, 0), rotation=(0, 45, 0))
