...
- **OpenMP**: Apple's default Clang doesn't include OpenMP. Install via `brew install libomp` for better performance
- **Performance**: Without OpenMP, the library will still function but with reduced parallelization capabilities

## Troubleshooting

### OpenMP Issues on macOS

**Problem**: `Could NOT find OpenMP_CXX (missing: OpenMP_CXX_FLAGS OpenMP_CXX_LIB_NAMES)`
...
2. **Set OpenMP environment variables**:
   ...
   export OpenMP_CXX_FLAGS="-Xpreprocessor -fopenmp"
   export OpenMP_CXX_LIB_NAMES="omp"
   ```

3. **Alternative: Use GCC with OpenMP**:

   ```bash
   brew install gcc
   export CXX=g++-13  # or whatever GCC version was installed
   export CC=gcc-13
   ```

4. **Build without OpenMP** (reduced performance):
   The CMake configuration will automatically fall back to a non-OpenMP build if OpenMP is not found.

### General Build Issues

**Problem**: CMake version too old
**Solution**: Update CMake via Homebrew: `brew upgrade cmake`

**Problem**: Missing C++ compiler
**Solution**: Install Xcode Command Line Tools: `xcode-select --install`

**Problem**: Python module import errors
**Solution**: Ensure Python dependencies are installed: `pip install -r requirements.txt`
