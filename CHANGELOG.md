...
- `sim_radar` takes `device="auto"` and defaults to it, selecting the GPU when one is usable and the CPU otherwise. `"auto"` is silent, because choosing the CPU there is the documented behaviour rather than a request that could not be honoured; `device="gpu"` still warns when it falls back, and that warning is now meaningful because it only fires when a caller asked for the GPU by name. Callers who passed nothing previously got `"gpu"`, so on a CPU-only build every call raised a fallback `RuntimeWarning`
