...
def cal_locations(radar, target):
    ...
    if type(location[0]) == str:
        temp_x = eval(
            location[0].replace('math', 'np').replace('t', 'timestamp'))
    ...
    if type(location[1]) == str:
        temp_y = eval(
            location[1].replace('math', 'np').replace('t', 'timestamp'))
    ...
    if type(location[2]) == str:
        temp_z = eval(
            location[2].replace('math', 'np').replace('t', 'timestamp'))
    ...
    if type(rcs) == str:
        rcs_t = eval(
            rcs.replace('math', 'np').replace('t', 'timestamp'))
    else:
        rcs_t = np.full_like(timestamp, rcs)

    if type(phase) == str:
        phs_t = eval(
            phase.replace('math', 'np').replace('t', 'timestamp'))
    else:
...
def cal_range_angle(radar, target):
    ...
    tg_to_tx_rng = np.sqrt((tgx_t - tx_tgx_t)**2 + (tgy_t - tx_tgy_t)**2 +
                           (tgz_t - tx_tgz_t)**2)
    tg_to_rx_rng = np.sqrt((tgx_t - rx_tgx_t)**2 + (tgy_t - rx_tgy_t)**2 +
                           (tgz_t - rx_tgz_t)**2)
...
def sim_target(radar, target):
    ...
    for i_tx in range(0, radar.transmitter.channel_size):
        for i_rx in range(0, radar.receiver.channel_size):
            ...
            tx_gain_ele = radar.transmitter.el_func[i_tx](
            ...
            rx_gain_ele = radar.receiver.el_func[i_rx](
                180 / np.pi * range_angle['tg_to_rx_ele'][ch, :, :]
            ) + radar.receiver.antenna_gains[i_rx]

            antenna_gain[ch, :, :] = tx_gain_azi + \
                tx_gain_ele+rx_gain_azi+rx_gain_ele

    pr_dbm = radar.transmitter.tx_power \
        + antenna_gain \
    ...
    peak_amp = (amp_mixer *
                10**(radar.receiver.baseband_gain / 20) *
                np.sqrt(2))

    delta_t = (range_angle['tg_to_tx_rng'] +
               range_angle['tg_to_rx_rng']) / const.c
...
def simpy(radar, targets, noise=True):
    ...
    warnings.warn('`simpy` has been deprecated, please use `simc` instead.',
    ...
    for n in range(0, target_number):
            ...
            for f_idx in range(0, channels):
                for p_idx in range(0, pulses):
                    pn_mod[f_idx, p_idx, :] = np.roll(
                        phase_noise[f_idx, p_idx, :],
                        shift_mean[f_idx, p_idx]) * \
            ...
            if radar.transmitter.waveform_mod[tx_idx]['enabled']:
                for i_pulse in range(0, pulses):
                    ...
                    for i_code in range(0, np.shape(phase_timestamp)[0]):
                        phase[i_ch, i_pulse,
                              np.where(temp_time > phase_timestamp[i_code]
        ...
        if len(k) == 2:
            baseband = baseband + pn_mod*raw_data['amplitude'] * np.exp(
                1j * (
                    2 * np.pi * (
                        (radar.f_offset_mat+f[0]+0.5*k[1]*beat_time)*beat_time -
                        (radar.f_offset_mat+f[0]+0.5*k[1]*(beat_time-delta_t)) *
                        (beat_time-delta_t)) +
                    raw_data['phs_t'] / 180 * np.pi
                )) * phase
        else:
            ...
            for p_idx in range(0, radar.transmitter.pulses):
                freq_t = (
                    f_offset[p_idx]+f[0:-1]+0.5*k[1:]*(d_pt[1:]))*(d_pt[1:])
                phase_t = np.cumsum(np.concatenate(([0], freq_t)))

                # Can't use linear interp here
                fun_phase_t = interp1d(
                    radar.t, phase_t, kind='linear',
                    bounds_error=False, fill_value=(phase_t[0], phase_t[-1]))
            ...
            baseband = baseband + pn_mod*raw_data['amplitude'] * np.exp(
                1j * (
                    2 * np.pi * (p0 - p1) + raw_data['phs_t'] / 180 * np.pi
                )) * phase
