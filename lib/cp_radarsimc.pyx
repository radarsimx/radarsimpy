...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef Transmitter[double] cp_Transmitter_double(radar,
                                         density):
    cdef int_t frames = radar.frames
    cdef int_t channles = radar.channel_size
    cdef int_t pulses = radar.transmitter.pulses
    ...
    return Transmitter[double](
        f_vect,
        f_offset_vect,
        t_vect,
        <double> radar.transmitter.tx_power,
        t_pstart_vect,
        t_frame_vect,
        <double> density,
        pn_vect
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef TxChannel[double] cp_TxChannel_double(tx,
                                     tx_idx):
    cdef int_t pulses = tx.pulses

    cdef vector[double] az_ang_vect, az_ptn_vect
    cdef vector[double] el_ang_vect, el_ptn_vect

    cdef vector[cpp_complex[double]] pulse_mod_vect

    cdef bool mod_enabled
    cdef vector[cpp_complex[double]] mod_var_vect
    cdef vector[double] mod_t_vect

    az_ang_vect.reserve(len(tx.az_angles[tx_idx]))
    for idx in range(0, len(tx.az_angles[tx_idx])):
        az_ang_vect.push_back(<double> np.radians(tx.az_angles[tx_idx][idx]))
    az_ptn_vect.reserve(len(tx.az_patterns[tx_idx]))
    for idx in range(0, len(tx.az_patterns[tx_idx])):
        az_ptn_vect.push_back(<double> (tx.az_patterns[tx_idx][idx]))
    ...
    for idx in range(0, pulses):
        pulse_mod_vect.push_back(cpp_complex[double](
            np.real(tx.pulse_mod[tx_idx, idx]), np.imag(tx.pulse_mod[tx_idx, idx])))

    mod_enabled = tx.waveform_mod[tx_idx]['enabled']
    if mod_enabled:
        for idx in range(0, len(tx.waveform_mod[tx_idx]['var'])):
            mod_var_vect.push_back(cpp_complex[double](
                np.real(tx.waveform_mod[tx_idx]['var'][idx]), np.imag(tx.waveform_mod[tx_idx]['var'][idx])))

        mod_t_vect.reserve(len(tx.waveform_mod[tx_idx]['t']))
        for idx in range(0, len(tx.waveform_mod[tx_idx]['t'])):
            mod_t_vect.push_back(<double> tx.waveform_mod[tx_idx]['t'][idx])

    return TxChannel[double](
        Vec3[double](
            <double> tx.locations[tx_idx, 0],
            <double> tx.locations[tx_idx, 1],
            <double> tx.locations[tx_idx, 2]
        ),
        Vec3[double](
            <double> tx.polarization[tx_idx, 0],
            <double> tx.polarization[tx_idx, 1],
            <double> tx.polarization[tx_idx, 2]
        ),
        az_ang_vect,
        az_ptn_vect,
        el_ang_vect,
        el_ptn_vect,
        <double> tx.antenna_gains[tx_idx],
        mod_t_vect,
        mod_var_vect,
        pulse_mod_vect,
        <double> tx.delay[tx_idx],
        <double> np.radians(tx.grid[tx_idx])
    )
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef RxChannel[double] cp_RxChannel_double(rx,
                                     rx_idx):
    cdef vector[double] az_ang_vect, az_ptn_vect
    cdef double[:] az_ang_mem, az_ptn_mem
    cdef vector[double] el_ang_vect, el_ptn_vect
    cdef double[:] el_ang_mem, el_ptn_mem
    ...
    return RxChannel[double](
        Vec3[double](
            <double> rx.locations[rx_idx, 0],
            <double> rx.locations[rx_idx, 1],
            <double> rx.locations[rx_idx, 2]
        ),
        Vec3[double](0, 0, 1),
        az_ang_vect,
        az_ptn_vect,
        el_ang_vect,
        el_ptn_vect,
        <double> rx.antenna_gains[rx_idx]
    )
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef Target[double] cp_Target_double(radar,
                               target,
                               shape):
    timestamp = radar.timestamp.astype(np.float64)
    cdef double[:, :] points_memview
    cdef uint64_t[:, :] cells_memview
    cdef double[:] origin

    cdef vector[Vec3[double]] c_loc_array
    cdef vector[Vec3[double]] c_speed_array
    cdef vector[Vec3[double]] c_rotation_array
    cdef vector[Vec3[double]] c_rotation_rate_array

    cdef double[:, :, :] tgx_t, tgy_t, tgz_t
    cdef double[:, :, :] sptx_t, spty_t, sptz_t
    cdef double[:, :, :] rotx_t, roty_t, rotz_t
    cdef double[:, :, :] rotratx_t, rotraty_t, rotratz_t

    cdef cpp_complex[double] ep, mu
    ...
    permittivity = target.get('permittivity', 'PEC')
    if permittivity == "PEC":
        ep = cpp_complex[double](-1, 0)
        mu = cpp_complex[double](1, 0)
    else:
        ep = cpp_complex[double](np.real(permittivity), np.imag(permittivity))
        mu = cpp_complex[double](1, 0)

    if np.size(location[0]) > 1:
        tgx_t = location[0].astype(np.float64)
    else:
        tgx_t = <double > location[0] + <double > speed[0]*timestamp

    if np.size(location[1]) > 1:
        tgy_t = location[1].astype(np.float64)
    else:
        tgy_t = <double > location[1] + <double > speed[1]*timestamp

    if np.size(location[2]) > 1:
        tgz_t = location[2].astype(np.float64)
    else:
        tgz_t = <double > location[2] + <double > speed[2]*timestamp
    ...
    return Target[double](& points_memview[0, 0],
                           & cells_memview[0, 0],
                           <int_t> cells_memview.shape[0],
                           Vec3[double](& origin[0]),
                           c_loc_array,
                           c_speed_array,
                           c_rotation_array,
                           c_rotation_rate_array,
